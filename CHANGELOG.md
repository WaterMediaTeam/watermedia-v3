...
- ⚙️ Changed: `GLEngine.Builder` now rejects a non-null `renderThread` without an executor at construction instead of failing later on submit
...
- ⚙️ Added: `HeadlessGFXEngine` — a permitted no-GPU `GFXEngine` that records uploads into memory instead of a context (server-side probing, CI, headless validation); exposes `uploadCount()`/`lastUpload()`/`lastFormat()`/`activeFrameTexture()` for pipeline introspection and replaces the former test-only stand-in
...
- ⚙️ Added: `JsonTool.parse(JsonElement, Class)` / `parse(String, Class)` / `write(Object)` — generic Gson binding helpers around one shared `Gson` instance, the single JSON entry point for the whole codebase
- ⚙️ Added: `NetRequest.fetchJson(Class, URI, Class)` (+ `String` overload) — the common REST resolver for platform handlers: JSON GET, 200 check, Gson bind and empty-body guard; every failure (bad status, empty body, transport or parse error) surfaces as a `PlatformException` tagged with the given handler class, keeping the root cause; handlers needing custom headers/methods keep building their own `NetRequest`
