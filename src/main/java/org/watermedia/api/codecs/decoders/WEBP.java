...
public class WEBP extends ImageCodec {
    ...
    @Override
    public ImageData decode(ByteBuffer buffer) throws XCodecException {
        LOGGER.debug(IT, "decode() called with buffer of size: {}", buffer.remaining());
        final WebPInfo info = RiffParser.parse(buffer);
        if (info == null) {
            throw new XCodecException("Failed to parse WebP");
        }
        ...
        if (info.hasAnimation()) {
            ...
            return new ImageData(new ByteBuffer[]{frame}, info.width(), info.height(), new long[]{0L}, 1L, ImageData.NO_REPEAT
            );
        }
    }

    private ImageData decodeAnimation(final ByteBuffer buffer, final WebPInfo info) throws XCodecException {
        ...
        if (animFrames.isEmpty()) {
            ...
            throw new XCodecException("No animation frames found");
        }
        ...
        final int repeat = (loopCount == 0) ? ImageData.REPEAT_FOREVER : loopCount;
        LOGGER.debug(IT, "Animation decoding complete - {} frames, repeat count: {}", animFrames.size(), repeat);

        return new ImageData(frames, canvasWidth, canvasHeight, delays, repeat);
    }
    ...
    private ByteBuffer decodeFrameToBgra(final ByteBuffer buffer, final RiffChunk bitstreamChunk,
                                          final RiffChunk alphaChunk, final int width, final int height) throws XCodecException {
        ...
        if (bitstreamChunk == null) {
            ...
            throw new XCodecException("No bitstream chunk");
        }
    ...
    private int[] decodeAnimFrameToBgra(final ByteBuffer buffer, final RiffParser.AnimFrame af) throws XCodecException {
