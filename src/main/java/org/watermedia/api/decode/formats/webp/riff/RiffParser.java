...
public final class RiffParser {
    ...
    public static WebPInfo parse(ByteBuffer buffer) throws DecoderException {
        ...
        if (buffer.remaining() < 8) {
            throw new DecoderException("Truncated WebP: missing chunk header");
        }
        ...
        return switch (firstChunk.fourCC()) {
            ...
            default ->
                    throw new DecoderException("Unknown WebP format: " + RiffChunk.fourCCString(firstChunk.fourCC()));
        };
    }

    private static RiffChunk readChunk(final ByteBuffer buffer, final int offset) throws DecoderException {
        if (offset + 8 > buffer.limit()) {
            throw new DecoderException("Truncated WebP: chunk header at offset " + offset);
        }
    ...
    private static WebPInfo parseLossy(final ByteBuffer buffer, final RiffChunk vp8Chunk, final int fileSize) throws DecoderException {
        ...
        if (vp8Chunk.size() < 10) {
            throw new DecoderException("VP8 chunk too small: " + vp8Chunk.size() + " bytes");
        }
        ...
        if (!isKeyFrame) {
            throw new DecoderException("VP8 must start with key frame");
        }
        ...
        if (sync0 != 0x9D || sync1 != 0x01 || sync2 != 0x2A) {
            throw new DecoderException("Invalid VP8 start code");
        }
    ...
    private static WebPInfo parseLossless(final ByteBuffer buffer, final RiffChunk vp8lChunk, final int fileSize) throws DecoderException {
        // VP8L HEADER: 1 byte signature (0x2F) + 4 bytes with dimensions
        final int dataOffset = vp8lChunk.dataOffset();
        if (vp8lChunk.size() < 5) {
            throw new DecoderException("VP8L chunk too small: " + vp8lChunk.size() + " bytes");
        }

        final int signature = buffer.get(dataOffset) & 0xFF;
        if (signature != 0x2F) {
            throw new DecoderException("Invalid VP8L signature: 0x" + Integer.toHexString(signature));
        }
        ...
        if (version != 0) {
            throw new DecoderException("Unsupported VP8L version: " + version);
        }
    ...
    private static WebPInfo parseExtended(final ByteBuffer buffer, final RiffChunk vp8xChunk, final int fileSize) throws DecoderException {
        ...
        if (bitstreamChunk == null && !hasAnimation) {
            throw new DecoderException("Missing VP8/VP8L bitstream in extended WebP");
        }
