...
import org.watermedia.api.decode.formats.webp.common.ColorCache;
...
public final class VP8LDecoder {
    ...
    public static int[] decode(final BitReader reader, final int width, final int height) throws DecoderException {
    ...
    public static ByteBuffer decodeToBgra(final BitReader reader, final int width, final int height) throws DecoderException {
        final int[] bgra = decode(reader, width, height);
        return DataTool.bgraToBuffer(bgra);
    }

    // DECODE WITHOUT HEADER (FOR ALPHA CHANNEL)
    public static int[] decodeNoHeader(final BitReader reader, final int width, final int height) throws DecoderException {
        return decode(reader, width, height);
    }

    private static Transform readTransform(final BitReader reader, final Transform.Type type, final int width, final int height) throws DecoderException {
        return switch (type) {
            case PREDICTOR -> {
    ...
    private static int[] decodeImage(final BitReader reader, final int width, final int height,
                                     final int colorCacheBits, final ColorCache cache) throws DecoderException {
    ...
    private static int[] decodeImageData(final BitReader reader, final int width, final int height,
                                         ...
                                         final ColorCache colorCache) throws DecoderException {
        ...
        while (pos < total) {
            ...
            if (code < 256) {
                ...
                if (colorCache == null) {
                    throw new DecoderException("Color cache code without color cache");
                }
