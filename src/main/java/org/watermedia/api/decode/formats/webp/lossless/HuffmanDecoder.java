...
import static org.watermedia.api.decode.formats.webp.lossless.VP8LDecoder.IT;
...
public final class HuffmanDecoder {
    ...
    public static HuffmanTable readTable(final BitReader reader, final int alphabetSize) throws DecoderException {
    ...
    public static HuffmanGroup readGroup(final BitReader reader, final int colorCacheSize) throws DecoderException {
        // ALPHABET SIZES:
    ...
    public static HuffmanGroup[] readGroups(final BitReader reader, final int numGroups, final int colorCacheSize) throws DecoderException {
    ...
    private static HuffmanTable readSimpleCode(final BitReader reader, final int alphabetSize) throws DecoderException {
        ...
        if (sym0 >= alphabetSize) {
            throw new DecoderException("Simple code symbol out of range: " + sym0);
        }
        ...
        if (numSymbols == 2) {
            ...
            if (sym1 >= alphabetSize) {
                throw new DecoderException("Simple code symbol out of range: " + sym1);
            }
        }
    ...
    private static HuffmanTable readNormalCode(final BitReader reader, final int alphabetSize) throws DecoderException {
        ...
        if (useLength) {
            ...
            if (maxSymbol > alphabetSize) {
                ...
                throw new DecoderException("Max symbol exceeds alphabet size");
            }
        } else {
        ...
        while (symbol < alphabetSize && tokensRemaining > 0) {
            ...
            if (code < 16) {
                ...
                if (symbol + repeat > alphabetSize) {
                    throw new DecoderException("Repeat code overflow: symbol=" + symbol + ", repeat=" + repeat + ", alphabetSize=" + alphabetSize);
                }
                ...
                final int repeat = 3 + extraBits;
                if (symbol + repeat > alphabetSize) {
                    throw new DecoderException("Repeat zero overflow: symbol=" + symbol + ", repeat=" + repeat + ", alphabetSize=" + alphabetSize);
                }
                symbol += repeat;
            } else if (code == 18) {
                // REPEAT ZERO (LONG)
