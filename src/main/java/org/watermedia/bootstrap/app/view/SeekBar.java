 ...
 * parent's width, paints a centered track, a translucent buffered range and the solid value range, and
 * carries a small square thumb at the current position. Pressing or dragging anywhere on the bar snaps
 * the value to the pointer, clamps it to {@code 0..1} and reports it through {@code onChange}.
 */
public final class SeekBar extends View<SeekBar> {
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        // THUMB — A 12px SQUARE CENTERED ON THE VALUE POSITION
        final int thumbX = x + Math.round(w * this.value);
        final int thumb = 12;
        canvas.fill(thumbX - thumb / 2, this.innerTop() + (this.innerHeight() - thumb) / 2, thumb, thumb, this.accent);
    }
