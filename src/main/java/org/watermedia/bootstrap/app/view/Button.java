 ...
 */
public final class Button extends View<Button> {
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        render(this.ctx.text, this.left, this.top, this.measuredWidth, this.measuredHeight,
    ...
    static void box(final int x, final int y, final int w, final int h,
                    final Color accent, final boolean filled, final boolean hover, final boolean enabled) {
        if (filled) {
            RenderSystem.fill(x, y, w, h, hover && enabled ? accent : AppTheme.alpha(accent, 210));
        } else {
            RenderSystem.fill(x, y, w, h, hover && enabled ? AppTheme.alpha(accent, 60) : AppTheme.alpha(AppTheme.BG_2, 220));
            RenderSystem.rect(x, y, w, h, enabled ? accent : AppTheme.alpha(accent, 90), hover ? 1.7f : 1.5f);
        }
        if (enabled) RenderSystem.glowRect(x, y, w, h, 0f, accent, hover ? 0.26f : 0.16f);
    }
     ...
     */
    public static void render(final TextRenderer text, final int x, final int y, final int w, final int h,
        ...
        render(text, x, y, w, h, label, subText, icon, subtextSpacing, accent, textColor, AppTheme.TEXT_FAINT, filled, hover, enabled);
    }
    ...
    public static void render(final TextRenderer text, final int x, final int y, final int w, final int h,
                              ...
                              final boolean hover, final boolean enabled) {
        box(x, y, w, h, accent, filled, hover, enabled);

        final Color tc = !enabled ? AppTheme.TEXT_FAINT : filled ? AppTheme.BG_0 : textColor;
        final int cy = y + h / 2;
        int cx = x + LEFT_PAD;
        if (icon != null && !icon.isEmpty()) {
            PixelIcon.draw(icon, cx, cy - ICON_SIZE / 2, ICON_SIZE, tc);
        ...
        if (chipW > 0) {
            RenderSystem.rect(chipX, cy - CHIP_H / 2, chipW, CHIP_H, AppTheme.STROKE, 1f);
            ...
            text.render(subText, chipX + CHIP_PAD, baseline - text.baselineOffset(AppTheme.TEXT_SUBTITLE), subTextColor, AppTheme.TEXT_SUBTITLE);
        }
        ...
        if (label != null && !label.isEmpty()) {
            ...
            text.renderBold(draw, cx, cy - text.glyphHeightBold(AppTheme.TEXT_BUTTON) / 2f, tc, AppTheme.TEXT_BUTTON);
        }
    }
}
