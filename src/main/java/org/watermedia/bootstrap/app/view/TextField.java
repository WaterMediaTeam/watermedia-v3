 ...
 * translucent background and a hairline border that turns the accent color while focused, and shows a
 * faint placeholder when empty. A click grabs focus, printable characters append (up to {@code maxLength})
 * and backspace drops the last character; every edit reports the new text through {@code onChange}. The
 * {@code focused} state is inherited from {@link View}.
 */
public final class TextField extends View<TextField> {
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        canvas.stroke(this.left, this.top, this.measuredWidth, this.measuredHeight,
                this.focused ? this.accent : AppTheme.STROKE, 1f);
