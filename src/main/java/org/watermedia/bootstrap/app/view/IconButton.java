 ...
 */
public final class IconButton extends View<IconButton> {
    ...
    protected void onDraw(final Canvas canvas) {
        render(this.left, this.top, this.measuredWidth, this.measuredHeight, this.icon, this.iconSize,
     ...
     */
    public static void render(final int x, final int y, final int w, final int h, final String icon,
        ...
        render(x, y, w, h, icon, iconSize, accent, iconColor, filled, hover, enabled, true);
    }
    ...
    public static void render(final int x, final int y, final int w, final int h, final String icon,
        ...
        if (bordered) Button.box(x, y, w, h, accent, filled, hover, enabled);
        if (icon != null && !icon.isEmpty()) {
            final Color ic = !enabled ? AppTheme.TEXT_FAINT : bordered && filled ? AppTheme.BG_0 : iconColor;
            PixelIcon.draw(icon, x + (w - iconSize) / 2, y + (h - iconSize) / 2, iconSize, ic);
        }
    }
}
