...
/**
 * Stacks its children on top of one another in the same box, like Android's {@code FrameLayout} — the
 * natural container for overlays: a dimmed scrim with a centered dialog on top. Each child is aligned by
 * its own {@link View#gravity} (interpreted in two dimensions: {@code CENTER} centers on both axes,
 * {@code LEFT}/{@code RIGHT} pin horizontally and center vertically, {@code TOP}/{@code BOTTOM} the
 * reverse); {@code MATCH_PARENT} on an axis fills it.
 */
public final class FrameLayout extends ViewGroup<FrameLayout> {
