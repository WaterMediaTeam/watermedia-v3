 ...
 * can be built without casts. Sizes use {@link #MATCH_PARENT} / {@link #WRAP_CONTENT} or an exact pixel
 * value, mirroring the Android layout model.
 *
 * @param <T> the concrete view type, for fluent chaining
 */
public abstract class View<T extends View<T>> {
    ...
    public final void measure(final int availWidth, final int availHeight) {
        final int boxW = this.width == MATCH_PARENT || this.width == WRAP_CONTENT ? availWidth : this.width;
        final int boxH = this.height == MATCH_PARENT || this.height == WRAP_CONTENT ? availHeight : this.height;
    ...
    // SUPPRESSION. TextField OVERRIDES IT; ViewGroup AGGREGATES OVER ITS CHILDREN.
